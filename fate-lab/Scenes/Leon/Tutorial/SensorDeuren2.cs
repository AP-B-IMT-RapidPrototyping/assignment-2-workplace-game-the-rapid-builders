...
public partial class SensorDeuren2 : Node3D
{
	...
	private void OnArea3DBodyEntered(Node3D body)
    {
		...
		if(_actie1 == 0)
		{
			...
			{
				_animLinks.Play("animatielinks2");
				_animRechts.Play("animatierechts2");
				_actie1++;
			}
		}
		else
    ...
    private void OnArea3DBodyExited(Node3D body)
    {
		if(_actie2 == 0)
		{
			if (body.IsInGroup("Player"))
			{
				...
				_animLinks.PlayBackwards("animatielinks2");
				_animRechts.PlayBackwards("animatierechts2");
				_actie2++;
			}
		}
		else
